...
class Setting {
  static registerCommand = (context: vscode.ExtensionContext) => {
    ...
    // 设置HTML内容
    const path = context.extensionPath + "/src/views/app/dist/index.html";

    return vscode.commands.registerCommand(Command.SETTING, async () => {
      // 创建并显示新的webview
      ...
      webViewPanel.webview.postMessage({
        ...
        data: File.getFileTemplateContent(context.extensionPath),
      });
    });
  };
}
